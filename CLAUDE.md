...
- `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` — optional, enables a secondary Telegram push notification from `/api/contact` and `/api/spark` (Postmark stays primary; Telegram failures are logged, never surfaced)
...
### Component layer
- Page sections are imported and stacked in `app/[locale]/page.tsx`. Current order: `Header → Hero → Spark → Work → Capabilities → Approach → About → Contact → Footer`. The page also renders a skip-to-main-content link before the header. Reordering or removing one means editing that page file.
...
- Both `/api/contact` and `/api/spark` additionally fire a **Telegram bot notification** via `lib/telegram.ts` (`sendTelegramMessage()`, direct `fetch` to `api.telegram.org/bot<TOKEN>/sendMessage`, HTML parse_mode). Best-effort, fire-and-forget after the Postmark send — failures (missing env, network, non-2xx) are logged but do not surface as 5xx. Pattern mirrored from the [Scintechn/pavineiva](https://github.com/Scintechn/pavineiva) site. Telegram-supported HTML subset is small (`<b> <i> <u> <s> <code> <pre> <a>`) — keep messages plain. Spark Telegram payload is a *summary* (contact, idea, elevator, COI range, stack/phase counts) since Telegram caps messages at 4096 chars — the full plan stays in the email (`lib/spark-telegram.ts`).
...
- **Spark scoping endpoint (`/api/spark` + `components/Spark.tsx`)**: hero-adjacent engagement device that turns a one-line product idea into a structured 4-week build plan via Claude Haiku 4.5 (OpenRouter). Visitor pastes idea + email-or-mobile → server validates → calls OpenRouter → schema-validates response → emails the plan to RECIPIENT_EMAIL (and to the lead when contact is an email) → renders the plan in the UI. The result panel's "Talk to us about this →" CTA pre-fills the contact form via sessionStorage.
  ...
  - **Daily $-cap**: `DAILY_BUDGET_USD = 5` in `lib/spark-budget.ts`; pre-flight `wouldExceedBudget` check + post-call `INCRBYFLOAT` (Upstash) or Map increment (in-memory). Returns 503 on exceed. **Runbook note**: OpenRouter-side token consumption on non-2xx responses is untracked by design (no token count returned to record), so actual spend may be very slightly under-counted under upstream-error storms.
  - **Cost of Inaction (COI)**: every plan ships with a `costOfInaction { currency, low, high, period, basis }` block; `currency` is server-detected from the contact (+55 → BRL, +351 → EUR, else USD) and locked through the validator so the model can't deviate. The validator requires the `basis` field to contain at least one non-zero digit (defends against hollow "no math" responses). Single-retry on any validation failure (model's schema compliance is brittle on the COI-loaded prompt — observed ~5–10% first-try miss rate, which retry rescues).
  ...
  - **Files**: `app/api/spark/route.ts`, `lib/spark-{types,security,budget,email}.ts`, `lib/prompts/spark.ts`, `lib/postmark.ts` (shared Postmark wrapper), `lib/html.ts` (shared escapeHtml extracted from contact route), `components/Spark.tsx`. Plan: `docs/spark-plan.md`. Postmark domain-verification runbook (DNS work for moving off single-sender): `docs/postmark-domain-runbook.md`.
