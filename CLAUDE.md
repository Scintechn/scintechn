...
- `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` — optional, enables a secondary Telegram push notification from `/api/contact` (Postmark stays primary; Telegram failures are logged, never surfaced)
...
- `/api/contact` additionally fires a **Telegram bot notification** via `lib/telegram.ts` (`sendTelegramMessage()`, direct `fetch` to `api.telegram.org/bot<TOKEN>/sendMessage`, HTML parse_mode). Best-effort, fire-and-forget after the Postmark send — failures (missing env, network, non-2xx) are logged but do not surface as 5xx. Pattern mirrored from the [Scintechn/pavineiva](https://github.com/Scintechn/pavineiva) site. Telegram-supported HTML subset is small (`<b> <i> <u> <s> <code> <pre> <a>`) — keep messages plain.
